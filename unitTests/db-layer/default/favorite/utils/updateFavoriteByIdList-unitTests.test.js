...
describe("updateFavoriteByIdList module", () => {
  let sandbox;
  let updateFavoriteByIdList;
  let FavoriteStub;
  ...
  beforeEach(() => {
    ...
    updateFavoriteByIdList = proxyquire(
      "../../../../../src/db-layer/main/Favorite/utils/updateFavoriteByIdList",
      {
  ...
  describe("updateFavoriteByIdList", () => {
    ...
    it("should return empty list if update returns no rows", async () => {
      ...
      const result = await updateFavoriteByIdList(["id99"], {
        status: "inactive",
      });
    ...
    it("should return list with one id if only one record updated", async () => {
      FavoriteStub.update.resolves([1, [{ id: "id1" }]]);

      const result = await updateFavoriteByIdList(["id1"], { active: false });
    ...
    it("should throw HttpServerError if model update fails", async () => {
      ...
      try {
        await updateFavoriteByIdList(["id1"], { name: "err" });
    ...
    it("should call update with empty dataClause", async () => {
      await updateFavoriteByIdList(["id1"], {});
      sinon.assert.calledOnce(FavoriteStub.update);
    });
  });
});
