...
describe("createCollectionDocument module", () => {
  let sandbox;
  let createCollectionDocument;
  let CollectionDocumentStub, ElasticIndexerStub, newUUIDStub;

  const fakeId = "uuid-123";
  const mockCreatedCollectionDocument = {
  ...
  beforeEach(() => {
    ...
    CollectionDocumentStub = {
      create: sandbox.stub().resolves(mockCreatedCollectionDocument),
    };
    ...
    createCollectionDocument = proxyquire(
      "../../../../../src/db-layer/main/CollectionDocument/utils/createCollectionDocument",
      {
  ...
  describe("createCollectionDocument", () => {
    it("should create CollectionDocument and index to elastic if valid data", async () => {
      const input = { id: "custom-id" };
      const result = await createCollectionDocument(input);

      expect(result).to.deep.equal({ id: fakeId, ...{ id: "custom-id" } });
      sinon.assert.calledOnce(CollectionDocumentStub.create);
    ...
    it("should throw HttpServerError wrapping BadRequestError if input has unexpected field", async () => {
      ...
      try {
        await createCollectionDocument(input);
    ...
    it("should throw HttpServerError if CollectionDocument.create fails", async () => {
      CollectionDocumentStub.create.rejects(new Error("DB error"));
      const input = { id: "custom-id" };
  ...
  describe("indexDataToElastic", () => {
    it("should call ElasticIndexer with collectionDocument data", async () => {
      const input = { id: "custom-id" };
      await createCollectionDocument(input);
      sinon.assert.calledOnce(ElasticIndexerStub);
