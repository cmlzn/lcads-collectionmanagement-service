...
describe("updateCollectionDocumentByIdList module", () => {
  let sandbox;
  let updateCollectionDocumentByIdList;
  ...
  beforeEach(() => {
    ...
    updateCollectionDocumentByIdList = proxyquire(
      "../../../../../src/db-layer/main/CollectionDocument/utils/updateCollectionDocumentByIdList",
      {
  ...
  describe("updateCollectionDocumentByIdList", () => {
    it("should return list of updated IDs if update is successful", async () => {
      const result = await updateCollectionDocumentByIdList(fakeIdList, {
        name: "Updated",
      });

      expect(result).to.deep.equal(["id1", "id2"]);
      sinon.assert.calledOnce(CollectionDocumentStub.update);
      const args = CollectionDocumentStub.update.getCall(0).args;
    ...
    it("should return empty list if update returns no rows", async () => {
      ...
      const result = await updateCollectionDocumentByIdList(["id99"], {
        status: "inactive",
      });
    ...
    it("should return list with one id if only one record updated", async () => {
      CollectionDocumentStub.update.resolves([1, [{ id: "id1" }]]);

      const result = await updateCollectionDocumentByIdList(["id1"], {
        active: false,
      });
    ...
    it("should throw HttpServerError if model update fails", async () => {
      ...
      try {
        await updateCollectionDocumentByIdList(["id1"], { name: "err" });
    ...
    it("should call update with empty dataClause", async () => {
      await updateCollectionDocumentByIdList(["id1"], {});
      sinon.assert.calledOnce(CollectionDocumentStub.update);
    });
  });
});
