...
describe("updateCurationHistoryById module", () => {
  let sandbox;
  let updateCurationHistoryById;
  let CurationHistoryStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated CurationHistory" };
  ...
  beforeEach(() => {
    ...
    updateCurationHistoryById = proxyquire(
      "../../../../../src/db-layer/main/CurationHistory/utils/updateCurationHistoryById",
      {
  ...
  describe("updateCurationHistoryById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateCurationHistoryById(fakeId, {
        name: "Updated",
      });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateCurationHistoryById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateCurationHistoryById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateCurationHistoryById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateCurationHistoryById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      CurationHistoryStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateCurationHistoryById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
