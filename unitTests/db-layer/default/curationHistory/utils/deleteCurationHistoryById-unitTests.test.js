...
describe("deleteCurationHistoryById module", () => {
  let sandbox;
  let deleteCurationHistoryById;
  let CurationHistoryStub, ElasticIndexerStub;
  let mockDocInstance;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted CurationHistory" }),
    };
    ...
    deleteCurationHistoryById = proxyquire(
      "../../../../../src/db-layer/main/CurationHistory/utils/deleteCurationHistoryById",
      {
  ...
  describe("deleteCurationHistoryById", () => {
    it("should delete CurationHistory and index removal in Elastic", async () => {
      const result = await deleteCurationHistoryById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted CurationHistory",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      CurationHistoryStub.findOne.resolves(localMock);

      const result = await deleteCurationHistoryById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteCurationHistoryById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteCurationHistoryById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      CurationHistoryStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteCurationHistoryById(fakeId).catch(
        (err) => err,
      );
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted CurationHistory" }),
      };
      CurationHistoryStub.findOne.resolves(localMock);

      const result = await deleteCurationHistoryById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted CurationHistory",
      });
