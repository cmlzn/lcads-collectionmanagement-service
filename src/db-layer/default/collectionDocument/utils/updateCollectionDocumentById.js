...
const updateCollectionDocumentById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const [rowsCount, [dbDoc]] = await CollectionDocument.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateCollectionDocumentById;
